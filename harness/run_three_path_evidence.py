...
MODE_PMEM = "lazy-pmem"
MODES = (MODE_BLK, MODE_BLK_SHARED, MODE_PMEM)
SOURCE_STATES = ("plaintext-cold", "plaintext-warm")
MATERIALIZATION_MAX_BYTES = 1024 * 1024
CACHE_BACKING = "file"
MODE_TAGS = {MODE_BLK: "b", MODE_BLK_SHARED: "s", MODE_PMEM: "p"}
STATE_TAGS = {"plaintext-cold": "c", "plaintext-warm": "w"}
MODE_PERMUTATIONS = (
    (MODE_BLK, MODE_BLK_SHARED, MODE_PMEM),
    (MODE_BLK_SHARED, MODE_PMEM, MODE_BLK),
    (MODE_PMEM, MODE_BLK, MODE_BLK_SHARED),
    (MODE_PMEM, MODE_BLK_SHARED, MODE_BLK),
    (MODE_BLK_SHARED, MODE_BLK, MODE_PMEM),
    (MODE_BLK, MODE_PMEM, MODE_BLK_SHARED),
)
SCRIPT_DIR = Path(__file__).resolve().parent
WORKER = SCRIPT_DIR / "run_benchmark_worker.py"


def mode_order(round_number: int, cell_index: int) -> tuple[str, str, str]:
    if round_number <= 0 or cell_index < 0:
        raise ValueError("round_number must be positive and cell_index non-negative")
    return MODE_PERMUTATIONS[(round_number - 1 + cell_index) % len(MODE_PERMUTATIONS)]
...
def build_run_contract(root: Path, rounds: int, vm_counts: list[int]) -> dict[str, object]:
    ...
    repositories = {
        ...
        "lazyd": source_revision(Path("/root/virtiolazyd/worktrees/lazyd-benchmark")),
    ...
    for round_number in range(1, args.rounds + 1):
        for source_state in SOURCE_STATES:
            for vm_count in args.vm_counts:
                for execution_order, mode in enumerate(
                    mode_order(round_number, cell_index), start=1
                ):
                    stem = (
